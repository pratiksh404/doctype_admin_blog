          ...
          <div class="card card-outline card-info">
            ...
            <div class="card-body pad">
              <div class="mb-3">
                ...
                <input type="text" name="author_id" class="form-control" id="post_author"
                  placeholder="Post Author" value="{{ Auth::User()->id }}" hidden>
                {{-- ---------- --}}
                {{-- Category --}}
                <label for="post_category">Post Category</label>
                <select id="post_category" name="category_id" class="form-control">
                  @foreach($categories as $category)
                    <option value="{{ $category->id }}" {{!empty($post) && $post->category_id ? ($category->id == $post->category_id ? 'selected' : '') : ''}}>{{ $category->name }}</option>
                  @endforeach
                </select>
                {{-- ---------- --}}
                {{-- Status --}}
              ...
              placeholder="Enter Post Slug" value="{{!empty($post) && $post->slug ? $post->slug : old('slug')}}">
               ...
               @endif
                ...
                <div class="form-check">
                 ...
                 <input class="form-check-input" type="checkbox" value="{{$tag}}" name="remove_tags[]" aria-label="...">{{$tag}} <br>
                  @endforeach
                ...
                <input type="checkbox" id="featured" name="featured" {{!empty($post) && $post->featured == 1 ? 'checked' : ''}} data-bootstrap-switch
        ...
        <div class="col-md-12">
          <div class="card card-outline card-info">
            ...
            <div class="card-body pad">
              ...
              <img src="{{asset('storage').'/'.$post->image}}" alt="{{$post->seo_title ? $post->seo_title : $post->title}}" class="img-fluid">
                  @endif
          ...
          <div class="card card-outline card-info">
            ...
            <div class="card-body pad">
              <div class="mb-3">
                ...
                <input type="text" name="seo_title" class="form-control" id="post_seo_title"
              placeholder="Enter SEO Title" value="{{!empty($post) && $post->seo_title ? $post->seo_title : ''}}">
                <br>
                {{-- Post Meta Description --}}
                <label for="post_meta_description">Post Meta Description</label>
                <textarea class="form-control" name="meta_description" rows="3" id="post_excerpt"
                  placeholder="Enter ...">
